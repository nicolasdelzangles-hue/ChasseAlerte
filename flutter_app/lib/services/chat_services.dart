...
sio.Socket? _socket;
  ...
  Future<List<dynamic>> getConversations() async {
    final r = await http.get(Uri.parse('$baseUrl/conversations'), headers: _headers);
    if (r.statusCode != 200) throw Exception('getConversations ${r.statusCode}');
    return jsonDecode(r.body) as List<dynamic>;
  }

Future<void> deleteConversation(int id) async {
    final r = await http.delete(Uri.parse('$baseUrl/conversations/$id'), headers: _headers);
    if (r.statusCode != 204 && r.statusCode != 200) {
      throw Exception(jsonDecode(r.body)['message'] ?? 'Suppression impossible');
    }
  }

  Future<List<dynamic>> getMessages(int conversationId, {int? before, int limit = 20}) async {
    final uri = Uri.parse('$baseUrl/messages/$conversationId').replace(queryParameters: {
  ...
  /// Crée (ou récupère) une conversation privée par téléphone. Renvoie { id, created? }
   Future<Map<String, dynamic>> createConversationByPhone(String e164) async {
    final r = await http.post(
      Uri.parse('$baseUrl/conversations/by-phone'),
      headers: _headers,
      body: jsonEncode({'phone': e164}),
    );
    if (r.statusCode != 200 && r.statusCode != 201) {
      throw Exception(jsonDecode(r.body)['message'] ?? 'Erreur création 1:1');
    }
  ...
  Future<Map<String, dynamic>> sendMessage(int conversationId, String body) async {
    ...
    final uri = Uri.parse('$baseUrl/users/by-phone')
        .replace(queryParameters: {'phone': e164});
    final r = await http.get(uri, headers: _headers);
  ...
  Future<Map<String, dynamic>> createGroup({
    ...
    if (r.statusCode != 201) {
      throw Exception(jsonDecode(r.body)['message'] ?? 'Erreur création groupe');
    }
  ...
  // ---------- Socket.IO ----------
  /// `socketUrl` = ex: http://localhost:3000
  ...
  // --- Listeners ---
  void onNewMessage(void Function(int conversationId, Map<String, dynamic> message) cb) {
    _socket?.off('message:new');
    _socket?.on('message:new', (data) {
      final map = Map<String, dynamic>.from(data);
      cb(map['conversationId'] as int, Map<String, dynamic>.from(map['message']));
    });
  }
  ...
  void onMessagesRead(void Function(int userId, int lastMessageId) cb) {
    _socket?.off('messages:read');
    _socket?.on('messages:read', (data) {
      final map = Map<String, dynamic>.from(data);
      cb(map['userId'] as int, (map['lastMessageId'] as num).toInt());
    });
  }
  ...
  void sendRead(int conversationId, int lastMessageId) {
    _socket?.emit('messages:read', {'conversationId': conversationId, 'lastMessageId': lastMessageId});
  }
}
